...
if library_asset.file.attached?
  ...
  # `file`: owner-only redirect, used for live preview + the editor's own
  # thumbnails -- a real, directly fetchable URL.
  # `extension`: lets the client build the *bare* `<id>.<ext>` fragment it
  ...
  # the <base> comments in projects_controller.rb / project.rb).
  json.file preview_asset_file_path(library_asset, format: extension)

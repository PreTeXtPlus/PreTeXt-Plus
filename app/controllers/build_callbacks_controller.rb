...
class BuildCallbacksController < ApplicationController
  ...
  def create
    ...
    when "success"
      ...
      build.mark!(:received_from_server, log: payload["log"].presence || "(No log returned from server.)")
      FullBuildArtifactJob.perform_later(build, artifact_url)
    when "failed"
      build.mark!(:failed, log: payload["log"].presence || "(No log returned from server.)")

class ProjectsController < ApplicationController
  ...
  def index
    ...
    if @query.present?
      ...
      # A combined search spans both buckets, so unlike the preview below it can't
      # tell from context alone which rows are shared -- :user is eager loaded so
      # _card's per-row access check doesn't add a query per row.
      @results = Project.accessible_to(current_user)
        .includes(:user, targets: [ :current_build, :latest_build ])
  ...
  private
    ...
    def project_params
      permitted = [
        :title, :docinfo, :use_common_docinfo, :visibility, :language, :description,

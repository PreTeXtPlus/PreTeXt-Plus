...
- **Editor menus** (`CodeEditorMenu.tsx`, `MenuDropdown.tsx`, `editorCommands.ts`, `editorConfigs/snippets.ts`): the code editor's **File / Edit / Insert / Tools** menu bar — the editor's only toolbar (there is no separate top bar above it). Every format gets the same menus in the same order — only their contents differ — with the format-specific document actions (Format PreTeXt, Import LaTeX, Clean up LaTeX, Edit Macros vs. Edit Preamble, Assets) grouped at the top of Tools, above the Monaco commands that are identical everywhere. File holds the document-level actions that don't fit a plain menu item's single click: "Document Properties…" opens a small dialog (`title`/`language`, committed via `onTitleChange`/`onLanguageChange` props) since neither an inline text field nor a `<select>` fits a dropdown row, plus Save and Cancel items (`onSaveButton`/`onCancelButton`, hidden when the host omits them). Presence avatars, the read-only badge, and the feedback link (`feedbackLink` prop, built by `Editors.tsx` from the store since the menu itself stays store-agnostic) render in the right-aligned strip alongside the format badge. "Convert to PreTeXt" stays a button rather than a menu item: it's the one action that changes what the project *is*.

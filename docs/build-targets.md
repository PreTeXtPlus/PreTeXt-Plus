...
payload carrying an `artifact_url` almost like a success — except it marks the build
`received_from_server_flagged` rather than plain `received_from_server`, so the warning
survives the async gap until `FullBuildArtifactJob` finishes the import.

That's a status word, not a boolean flag on `success` — deliberately, this time. The
naive version of an "eighth status" would mean teaching every place that asks "is this
build usable?" (`prune_builds!`, the history table, the preview and download links) a
second spelling of success. Instead, `Build::SUCCESSFUL_STATUSES` /
`Build.successful` / `#successful?` is the **one** place that list lives — every one of
those call sites asks `successful?` once and doesn't care which of the three statuses
answered yes. `success?` itself (generated by the enum) keeps meaning only the clean
case, so a caller that doesn't know about the flagged statuses fails safe: it simply
under-counts rather than silently mixing an unaccepted build into what's usable.
`FullBuildArtifactJob` picks between `success` and `success_awaiting_review` based on
whether the build it's importing was flagged, read off `status` before the completing
`mark!` overwrites it.

`Build#built_with_errors?` is `success_awaiting_review? || success_errors_accepted?` — a
build whose *import* later fails or stalls never reaches either status, and ends up
simply `failed` with no output at all.

#### Going live is opt-in

Importing is not publishing. `success_awaiting_review` output does **not** become
`current_build` on its own, because a rebuild that reports errors must not silently
replace working published output with output nobody has looked at.
`Build.live_candidates` is the scope `sync_from_builds!` picks `current_build` from —
`success` and `success_errors_accepted`, explicitly excluding `success_awaiting_review` —
and `previous_successful_build` reads the same scope, so restoring is genuinely one step
behind what is live.

Until then the build is `success_awaiting_review` (`awaiting_review?`): imported,
previewable, live nowhere. The author
...
| `builds.status` `received_from_server_flagged`/`success_awaiting_review`/`success_errors_accepted` | enum values, no schema change | Added on top of the original eight to carry "the build server reported errors but still produced output" through, respectively: the async gap before import, imported-but-not-yet-accepted, and author-accepted. `Build::SUCCESSFUL_STATUSES`/`#successful?` is the one place that treats them as a group; `Build.live_candidates` — and therefore `current_build` — excludes `success_awaiting_review` explicitly. |
...
- **`builds.status` is an integer enum, so new values are additive only.** `canceled` is 6
  ...
  existing row. `received_from_server_flagged`/`success_awaiting_review`/
  `success_errors_accepted` (8/9/10) are the same trick applied to replace the
  `completed_with_errors`/`errors_accepted` booleans — no schema change to add them, only
  to drop the columns they replaced.
